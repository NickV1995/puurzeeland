...
//include_once REVIEW_PLUGIN_INCLUDES_VIEWS_DIR . '/review_plugin_main_view.php';

class Review{

  public $goedgekeurd = '';
  public $nr_of_reviews_per_page = 5; 
...
public function getLabel(){
  ...
  public function getTotalNumberOfPages(){
    ...
    $result = $this->getNumberOfApprovedReviews();

    $total_rows = $result;
        
    $total_pages = ceil($total_rows / $this->nr_of_reviews_per_page);
  ...
  public function getLimitRows(){
    ...
    $offset = ($pageno-1) * $this->nr_of_reviews_per_page;

    $query = "SELECT * FROM " . $this->getTableName() . "WHERE goedgekeurd='Ja' LIMIT $offset, $this->nr_of_reviews_per_page";
  ...
  public function goedkeuren($input_array){
    ...
    try{
      ...
      $wpdb->query($wpdb->prepare("UPDATE ".$this->getTableName(). " SET `goedgekeurd` = 'Ja' ". "WHERE
`wp_review`.`review_id` =%d;", $input_array['id']) );
